...
void setup() {
  ...
  //webSocket.on("esp_sendRequest_DoAm", socket_DoAm);
...
void SendRequest()
{
  ...
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin("http://192.168.1.253:3000/device/" + String(macaddress) + "?TrangThaiQuat=" + String(turnQuat) + "&DoAm=" + deviceDoAm()  + "&CO2=" + String(deviceOXI()));//"?dateTime=" + data_time()
...
String data_time() {
  ...
  return  timeWeekDay + formattedDate;
